        ...
        public IEP(int? stid = null, int? iepId = null)
        {
            if (stid != null)
            {
                ...
                if (current != null)
                {
                    ...
                    isGoalCompleted = db.tblGoals.Where(g => g.IEPid == current.IEPid) != null ? db.tblGoals.Where(g => g.IEPid == current.IEPid).ToList().All(g => g.Completed) : false ;
                    isServiceCompleted = db.tblServices.Where(s => s.IEPid == current.IEPid) != null ? db.tblServices.Where(s => s.IEPid == current.IEPid).ToList().All(s => s.Completed) : false ;
                }
            }
        }
