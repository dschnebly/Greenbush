...
namespace GreenbushIep.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult MISPortal()
        {
            ...
            if (MIS != null)
            {
                PortalViewModel model = new PortalViewModel
                {
                    ...
                    members = db.uspUserList(MIS.UserID, null, null, null, false).Select(u => new StudentIEPViewModel() { UserID = u.UserID, FirstName = u.FirstName, LastName = u.LastName, MiddleName = u.MiddleName, RoleID = u.RoleID, KidsID = u.KIDSID.ToString(), StatusActive = u.StatusActive, StatusCode = u.StatusCode, hasIEP = u.hasIEP ?? false }).ToList()
                };
        ...
        public ActionResult AdminPortal(int? userId)
        {
            ...
            if (ADMIN != null)
            {
                PortalViewModel model = new PortalViewModel
                {
                    ...
                    members = db.uspUserList(ADMIN.UserID, null, null, null, false).Select(u => new StudentIEPViewModel() { UserID = u.UserID, FirstName = u.FirstName, LastName = u.LastName, MiddleName = u.MiddleName, RoleID = u.RoleID, KidsID = u.KIDSID.ToString(), StatusActive = u.StatusActive, StatusCode = u.StatusCode, hasIEP = u.hasIEP ?? false }).ToList()
                };
        ...
        public ActionResult IEPFormFile(int id, string fileName)
        {
            ...
            else if (fileName == "ChildOutcomesSummary")
            {
        ...
        private IEP GetIEPPrint(int stid, int iepId)
        {
            ...
            if (theIEP != null && theIEP.current != null)
            {
                ...
                if (info != null && theIEP.current != null)
                {
                    ...
                    {
                        DateTime iepDate = theIEP.iepStartTime ?? theIEP.current.begin_date.Value;
        ...
        private void SaveFormValues(string HTMLContent, string formName, string studentId)
        {
            ...
            else if (formNameStr == "SUMMARY OF PERFORMANCE")
            {
                ...
                Int32.TryParse(exitYear, out int exitYearVal);
                if (exitYearVal > 0)
            ...
            else if (formNameStr == "CHILD OUTCOMES SUMMARY")
            {
