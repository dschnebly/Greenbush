...
namespace GreenBushIEP.Reports.ProgressReport
{
    public partial class Report : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                GreenBushIEP.Report.ReportMaster.StatusList(statusDD);
        ...
        private void ShowReport()
        {
            ...
            System.Collections.Generic.List<TeacherView> teacherList = GreenBushIEP.Report.ReportMaster.GetTeachers(User.Identity.Name);


            foreach (TeacherView teacher in teacherList)
            {
        ...
        private DataTable GetData(string districtFilter, string providerIds, string buildingID, string status, string teacherIds, string studentIds)
        {
            ...
            using (IndividualizedEducationProgramEntities ctx = new IndividualizedEducationProgramEntities())
            {
                ...
                System.Data.Entity.Core.Objects.ObjectResult<up_ReportProgress_Result> list = ctx.up_ReportProgress(districtFilter, status, buildingID, providerIds, teacherIds, studentIds);

                foreach (up_ReportProgress_Result cs in list)
                {
